...
export default function Sidebar() {
  ...
  // Accordion Menu State (Open/Close)
      ...
      if (session?.user?.email) {
        ...
        if (profile) {
          if (profile.modules) {
            ...
            // Default access if modules are empty
    ...
    <>
      ...
      <div className={`fixed top-0 left-0 h-screen w-72 bg-white flex flex-col shadow-2xl border-r border-slate-200 z-[100] transform transition-transform duration-300 ease-in-out ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <nav className="flex-1 px-4 py-6 overflow-y-auto custom-scrollbar space-y-2">
          ...
          {/* SECTION 3: MAIN OPERATIONS */}
          {(allowedModules.includes("leads") || allowedModules.includes("inventory")) && (
            <div className="mb-2">
              <button onClick={() => toggleMenu('operations')} className="w-full flex items-center justify-between px-4 py-3 rounded-xl hover:bg-slate-50 transition-colors group outline-none">
                <span className="text-xs font-extrabold text-slate-500 uppercase tracking-widest group-hover:text-emerald-600 transition-colors">Main Operations</span>
                <svg className={`w-4 h-4 text-slate-400 group-hover:text-emerald-500 transition-transform duration-300 ${expanded.operations ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M19 9l-7 7-7-7" /></svg>
              </button>
          ...
          {(allowedModules.includes("hr") || allowedModules.includes("tasks") || allowedModules.includes("account") || allowedModules.includes("settings")) && (
            <div className="mb-2">
              <button onClick={() => toggleMenu('management')} className="w-full flex items-center justify-between px-4 py-3 rounded-xl hover:bg-slate-50 transition-colors group outline-none">
                <span className="text-xs font-extrabold text-slate-500 uppercase tracking-widest group-hover:text-orange-600 transition-colors">Management</span>
                <svg className={`w-4 h-4 text-slate-400 group-hover:text-orange-500 transition-transform duration-300 ${expanded.management ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M19 9l-7 7-7-7" /></svg>
              </button>
